...
**sort**: 已改为外部排序，不再要求全量加载内存。可通过 `--max-memory` 控制分块内存预算，并可通过 `--threads` 指定排序线程数。

**dedup**: 已改为磁盘分桶去重，桶内维护 `HashSet`，并在最终阶段按输入序号多路归并，整体内存可控。
...
- `sort`：`rayon::par_sort_by` 并行排序每个块，支持 `--threads` 指定本地线程池线程数
- `search`：正链/反链匹配使用 `rayon::join` 并行
