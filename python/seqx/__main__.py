...
def find_seqx_bin() -> str:
    """Return the seqx binary path."""

    seqx_exe = "seqx" + sysconfig.get_config_var("EXE")

    scripts_path = os.path.join(sysconfig.get_path("scripts"), seqx_exe)
    ...
    user_path = os.path.join(
        sysconfig.get_path("scripts", scheme=user_scheme), seqx_exe
    )
    ...
    target_path = os.path.join(pkg_root, "bin", seqx_exe)
    if os.path.isfile(target_path):
        return target_path

    # Search for pip-specific build environments.
    #
    # Expect to find seqx in <prefix>/pip-build-env-<rand>/overlay/bin/seqx
    ...
    if len(paths) >= 2:
        ...
        if (
            ...
            # The overlay must contain the seqx binary.
            candidate = os.path.join(paths[0], seqx_exe)
...
if __name__ == "__main__":
    seqx = os.fsdecode(find_seqx_bin())
    if sys.platform == "win32":
        import subprocess

        completed_process = subprocess.run([seqx, *sys.argv[1:]])
        sys.exit(completed_process.returncode)
    else:
        os.execvp(seqx, [seqx, *sys.argv[1:]])